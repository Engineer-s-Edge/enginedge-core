...
Write-Host 'Tearing down: Stateful Backend, Messaging, Core Applications, Scheduling App, News Ingestion Job, Observability'
...
if (helm status kube-prometheus-stack --namespace default 2>$null) { helm delete kube-prometheus-stack --namespace default }
...
Write-Host 'Preserving PersistentVolumeClaims: wolfram-state'
# PVC 'wolfram-state' is preserved and will not be deleted
Write-Host 'PersistentVolumeClaims preserved (wolfram-state and other PVCs retained).'
