...
export class IdentityClientService {
  ...
  async login(email: string, password: string) {
    const { data } = await axios.post(`${this.baseUrl}/internal/auth/login`, {
      email,
      password,
    });
    return data;
  ...
  async refresh(refreshToken: string) {
    const { data } = await axios.post(
      `${this.baseUrl}/internal/auth/token/refresh`,
      {
        refreshToken,
      },
    );
  ...
  async revoke(refreshToken: string) {
    const { data } = await axios.post(
      `${this.baseUrl}/internal/auth/token/revoke`,
      {
        refreshToken,
      },
    );
  ...
  async oauthAuth(provider: string) {
    const { data } = await axios.get(
      `${this.baseUrl}/internal/oauth/${provider}/auth`,
    );
    return data;
  }
  ...
  async oauthCallback(provider: string, code: string, state?: string) {
    const { data } = await axios.get(
      `${this.baseUrl}/internal/oauth/${provider}/callback`,
      {
        params: { code, state },
      },
    );
  ...
  async oauthUnlink(provider: string, userId: string) {
    const { data } = await axios.delete(
      `${this.baseUrl}/internal/oauth/${provider}/unlink`,
      {
        params: { userId },
      },
    );
