    ...
    async (req: IncomingMessage, socket: Duplex, head: Buffer) => {
      ...
      const routes = [
        ...
        },
        {
          ...
          base:
            process.env.SCHEDULING_WORKER_URL ||
            'http://scheduling-worker:3000',
        },
        {
      ...
      const match = routes.find((r) =>
        r.prefixes.some((p) => url.startsWith(p)),
      );
      ...
      wss.handleUpgrade(req, socket as any, head, (wsClient: WebSocket) => {
        ...
        wsClient.on(
          'message',
          (data: Buffer) =>
            wsTarget.readyState === WebSocket.OPEN && wsTarget.send(data),
        );
        wsTarget.on(
          'message',
          (data: Buffer) =>
            wsClient.readyState === WebSocket.OPEN && wsClient.send(data),
        );
