...
export class TimeManagementService {
  constructor(
    ...
    this.logger.info(
      'TimeManagementService initialized',
      TimeManagementService.name,
    );
  }

  async getDailyTimeBreakdown(userId: string): Promise<DailyTimeBreakdown> {
    this.logger.info(
      `Getting daily time breakdown for user: ${userId}`,
      TimeManagementService.name,
    );
    try {
      ...
      this.logger.info(
        `Daily time breakdown for user ${userId}: ${breakdown.total} minutes total (${breakdown.habits} habits, ${breakdown.goals} goals)`,
        TimeManagementService.name,
      );
      return breakdown;
    } catch (error: unknown) {
      ...
      this.logger.error(
        `Failed to get daily time breakdown for user: ${userId}`,
        e.stack,
        TimeManagementService.name,
      );
      throw e;
