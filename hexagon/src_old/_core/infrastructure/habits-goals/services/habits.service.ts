...
export class HabitsService {
  ...
  async findOne(id: string, userId: string): Promise<Habit> {
    ...
    try {
      ...
      if (!habit) {
        this.logger.warn(
          `Habit with ID ${id} not found for user: ${userId}`,
          HabitsService.name,
        );
  ...
  async toggleEntry(
    ...
    this.logger.info(
      `Toggling habit entry - habitId: ${id}, userId: ${userId}, date: ${entryData.date}`,
      HabitsService.name,
    );
    ...
    this.logger.info(
      `Entry date parsed: ${entryDate.toISOString()}`,
      HabitsService.name,
    );
    this.logger.info(
      `Existing entries count: ${habit.entries.length}`,
      HabitsService.name,
    );

    // Find existing entry for this date using a more reliable date comparison
    const targetDateString = entryDate.toISOString().split('T')[0];
    ...
    this.logger.info(
      `Looking for date: ${targetDateString}, found existing entry at index: ${existingEntryIndex}`,
      HabitsService.name,
    );
    ...
    if (existingEntryIndex >= 0) {
      ...
      this.logger.info(
        `Updating existing entry from completed: ${habit.entries[existingEntryIndex].completed} to: ${entryData.completed}`,
        HabitsService.name,
      );
      ...
      // Create new entry
      this.logger.info(
        `Creating new entry with completed: ${entryData.completed ?? false}`,
        HabitsService.name,
      );
    ...
    const savedHabit = await habit.save();
    this.logger.info(
      `Habit saved successfully. Total entries: ${savedHabit.entries.length}`,
      HabitsService.name,
    );
    return savedHabit;
  }
  ...
  async getTotalDailyTimeCommitment(userId: string): Promise<number> {
    ...
    return activeHabits.reduce(
      (total, habit) => total + (habit.dailyTimeCommitment || 0),
      0,
    );
  }
