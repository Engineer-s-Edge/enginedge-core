...
describe('ProxyService', () => {
  ...
  beforeEach(async () => {
    ...
    // Manual instantiation since @nestjs/testing is not available
    service = new ProxyService();

    mockedAxios.request = jest.fn().mockResolvedValue({ data: { success: true } });
  });
  ...
  describe('forward', () => {
    it('should forward request to backend', async () => {
      ...
      const method = 'POST';
      const body = { foo: 'bar' };
      const headers = { authorization: 'Bearer token', 'custom-header': 'ignored' };
    ...
    it('should filter security headers', async () => {
      const headers = {
        authorization: 'auth',
        'x-request-id': '123',
        cookie: 'session=abc', // Should be removed? Not in filter list
        host: 'localhost', // Removed
      };

      await service.forward('http://base', 'path', 'GET', {}, headers, {});
